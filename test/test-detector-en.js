...
function refEq(r, book, chapter, vs, ve) {
  return r && r.book === book && r.chapter === chapter
    && r.verseStart === vs && r.verseEnd === (ve === undefined ? vs : ve);
}
...
assert(refEq(detector.detect('John chapter 3 verses 16 to 18'), 'jean', 3, 16, 18), 'John chapter 3 verses 16 to 18');
...
assert(psOnly && psOnly.book === 'psaumes' && psOnly.chapter === 23 && psOnly.verseStart === undefined,
...
assert(refEq(detector.detect('First Corinthians 13:4-7'), '1corinthiens', 13, 4, 7), 'First Corinthians 13:4-7');
assert(refEq(detector.detect('II Timothy 3:16'), '2timothee', 3, 16), 'II Timothy 3:16 (numéral romain)');
