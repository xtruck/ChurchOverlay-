...
Build order below, by risk and dependency — each still needs its own dedicated architecture note before its code starts (ARCHITECTURE.md section 59.3). Voice-driven verse navigation and NDI output were both added mid-session as extensions beyond the original three Phase 2 items; voice navigation is sequenced right after media library since both are architecture-note-complete and ready to build, while NDI output (also architecture-note-complete) is sequenced after rundown/scenes since it is explicitly NOT yet cleared for implementation pending real verification of its native dependency's licensing and platform support (section 62.3):
