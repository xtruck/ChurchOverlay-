...
(async () => {
  ...
  check('le payload est marqué detectedBy: quote', !!shown && shown.detectedBy === 'quote');
  ...
  // server.js consulte findByQuotedText() une fois en détection (STEP 3),
  // puis une seconde fois lors de la récupération du texte du verset — mais
  // seulement si le segment franchit la vérification anti-doublon. Segment 1
  // (nouveau) : 2 appels. Segment 2 (doublon, court-circuité par le dédoublonnage
  // avant le second appel) : 1 appel. Total attendu : 3.
  check('findByQuotedText a bien été consulté sur les deux segments', quoteLookups === 3, `appels=${quoteLookups}`);
